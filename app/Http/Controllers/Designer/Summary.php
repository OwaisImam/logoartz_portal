...
class Summary extends DesignerController {
...
//       return view('admin.summary.NewOrders', $this->data);
    ...
    public function search_order() {
        ...
        $DesignerID = \Session::get('DesignerID');
        $OrderNo = 0;
        $QuoteNo = 0;        
        $DesignName = "";
        ...
        if (!empty($DigitizingResult)) {
            foreach ($DigitizingResult as $digitRes) {
                $CompleteResult[] = [
                    ...
                    'OrderRef' => 'Digitizing',
                    'OrderDetailLink' => url('designer/digi/details/' . $digitRes->OrderID),
        ...
        $Vector = DB::table('vector_order')
                ->leftjoin('customers', 'customers.CustomerID', '=', 'vector_order.CustomerID')
                ->leftjoin('designers', 'designers.DesignerID', '=', 'vector_order.DesignerID')
                ->where('vector_order.DesignerID', $DesignerID);
        if ($OrderNo != 0) {
        ...
        if (!empty($VectorResult)) {
            foreach ($VectorResult as $vectRes) {
                $CompleteResult[] = [
                    ...
                    'OrderRef' => 'Vector',
                    'OrderDetailLink' => url('designer/vector/details/' . $vectRes->VectorOrderID),
        ...
        return view('designer.summary.SearchDetail', $this->data);
