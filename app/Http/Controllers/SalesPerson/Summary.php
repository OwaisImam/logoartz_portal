...
class Summary extends SalesPersonController {
    ...
    public function index() {
        ...
        return view('salesperson.summary.history.historyindi', $this->data);
    }

    public function summary_view()
    {
        $sales_id = \Session::get('SalesPersonID'); 
        $this->data['allcustomers'] = \App\Customers::where('SalesPersonID', $sales_id)->get();
        $this->data['Orders'] = '';
        $this->data['OrderStatuses'] = Config('order_statuses');
        $this->data['OrderTypes'] = Config('order_types');

        return view('salesperson.history.index', $this->data);
    }
            ...
            if ($Cat == 0) {
                ...
                $this->data['Cat'] = 0;
                 ...
                 $Qurey = \App\DigiOrders::select('digitizing_orders.OrderID', 'CustomerName', 'DesignerName', 'DesignName', 'PONumber', 'OrderType', 'digitizing_orders.IsRead', 'digitizing_orders.OrderStatus')
                        ...
                        ->where('digitizing_orders.SalesPersonID', $sales_id)
                ...
                $this->data['Cat'] = 1;
                   ...
                   $Qurey = \App\vector_order::select('VectorOrderID', 'CustomerName', 'DesignerName', 'DesignName', 'PONumber', 'vector_order.Status', 'OrderType', 'vector_order.IsRead', 'vector_order.OrderStatus')
                        ...
                        ->leftjoin('designers', 'designers.DesignerID', '=', 'vector_order.DesignerID')
                         ->where('vector_order.SalesPersonID', $sales_id)
    ...
    $sales_id = \Session::get('SalesPersonID'); 
        if ($sales_id > 0){
         ...
         $this->data['customers'] = Customers::where('SalesPersonID', $sales_id)->get();
         return view('salesperson.accounts.index', $this->data);

        } else{

            return redirect('/salesperson/dashboard')->with('warning_msg', "Invalid Sales Person");
        }
    ...
    $To  = Input::get('DateTo');
    $From = Input::get('DateFrom');
    $cus_id = Input::get('cusname');
    $cetagory = Input::get('cetagory');
    $sales_id = \Session::get('SalesPersonID'); 
    $this->data['OrderStatuses'] = Config('order_statuses');
    $this->data['OrderTypes'] = Config('order_types');
    $this->data['customers'] = Customers::where('SalesPersonID', $sales_id)->get();
        ...
        if ($To != "" && $From != ""){
            if ($cetagory != "" && $cetagory == 1){

                // Digitizing

                 $this->data['Cat'] = 1;
                            $Qurey = \App\DigiOrders::
                              select('digitizing_orders.OrderID', 'CustomerName', 'DesignerName', 'DesignName',
                                     'PONumber', 'OrderType', 'SalesPersonName', 'SalesPrice','digitizing_orders.IsRead')
                            ->leftjoin('customers', 'customers.CustomerID', '=', 'digitizing_orders.CustomerID')
                            ->leftjoin('designers', 'designers.DesignerID', '=', 'digitizing_orders.DesignerID')
                        ...
                        if ($cus_id > 0){
                              $Qurey->where('digitizing_orders.CustomerID', $cus_id);
                        } else{
                              $Qurey->where('digitizing_orders.SalesPersonID', $sales_id);
                        }
            ...
            } elseif($cetagory != "" && $cetagory == 2){

                  // Vector

                   $this->data['Cat'] = 2;
                            $Qurey = \App\vector_order::
                              select('vector_order.VectorOrderID', 'CustomerName', 'DesignerName', 'DesignName',
                                     'PONumber', 'OrderType', 'SalesPersonName', 'SalesPrice','vector_order.IsRead')
                            ->leftjoin('customers', 'customers.CustomerID', '=', 'vector_order.CustomerID')
                            ->leftjoin('designers', 'designers.DesignerID', '=', 'vector_order.DesignerID')
                        ...
                        if ($cus_id > 0){
                              $Qurey->where('vector_order.CustomerID', $cus_id);
                        } else{
                              $Qurey->where('vector_order.SalesPersonID', $sales_id);
                        }

                $this->data['v_orders'] = $Qurey->orderby('vector_order.VectorOrderID', 'desc')->get();

               // dd($this->data['v_orders']);
                ...
                // Digitizing Data Patching

                $d_Qurey = \App\DigiOrders::
                              select('digitizing_orders.OrderID', 'CustomerName', 'DesignerName', 'DesignName',
                                     'PONumber', 'OrderType', 'SalesPersonName', 'SalesPrice','digitizing_orders.IsRead')
                            ->leftjoin('customers', 'customers.CustomerID', '=', 'digitizing_orders.CustomerID')
                            ->leftjoin('designers', 'designers.DesignerID', '=', 'digitizing_orders.DesignerID')
                            ...
                            ->where('digitizing_orders.OrderType', '!=', 4);
                        if ($cus_id > 0){
                              $d_Qurey->where('digitizing_orders.CustomerID', $cus_id);
                        } else{
                              $d_Qurey->where('digitizing_orders.SalesPersonID', $sales_id);
                        }


                // Vector Data Patching
                
                $v_Qurey = \App\vector_order::
                              select('vector_order.VectorOrderID', 'CustomerName', 'DesignerName', 'DesignName',
                                     'PONumber', 'OrderType', 'SalesPersonName', 'SalesPrice','vector_order.IsRead')
                            ->leftjoin('customers', 'customers.CustomerID', '=', 'vector_order.CustomerID')
                            ->leftjoin('designers', 'designers.DesignerID', '=', 'vector_order.DesignerID')
                            ...
                            ->where('vector_order.OrderType', '!=', 4);
                        if ($cus_id > 0){
                              $v_Qurey->where('vector_order.CustomerID', $cus_id);
                        } else{
                              $v_Qurey->where('vector_order.SalesPersonID', $sales_id);
                        }
        ...
        return redirect('salesperson/accounts_summary')->with('warning_msg', "Invalid Selecttion");
    ...
    public function search_records() {
        ...
        if (!empty($To || $From)) {
                ...
                $this->data['Orders'] = \App\DigiOrders::select('digitizing_orders.OrderID', 'CustomerName', 'DesignerName', 'DesignName', 'PONumber', 'OrderType', 'digitizing_orders.Status', 'digitizing_orders.IsRead')
                        ->leftjoin('customers', 'customers.CustomerID', '=', 'digitizing_orders.SalesPersonID')
                        ->leftjoin('designers', 'designers.DesignerID', '=', 'digitizing_orders.SalesPersonID')
                        ->whereRaw('digitizing_orders.DateAdded BETWEEN "' . $From . '" AND "' . $To . '"')
                        ->where('digitizing_orders.Status', 7)
               ...
               return view('designer.summary.history.historyindi', $this->data);
               ...
               $type = "Order";
                 ...
                 $mailFrom = 'technical-team@logoartz.com';
                  ...
                  "Adminmsg" => \Input::get('MessageForDesigner')
                    ...
                    , function($message) use ($mailFrom) {
                $message->to('umerbhattiboostani@gmail.com')->from($mailFrom, 'Logo Artz')->subject('Logo Artz -Assigned Digitizing Order');
            });
...
// echo '<pre>'.print_r($VOrderIds, 1).'</pre>'; die;
            ...
            if(!empty($DOrdersData || $VOrderData)) {
               foreach($DOrdersData as $Price_str)
                 {
                $DPrices = $DPrices + $Price_str->Price;
                 }

                foreach($VOrderData as $Price_str)
                 {
        ...
        if (Input::has('dorderids') || Input::has('vorderids') && Input::has('CustomerID')) {
          ...
          $fileName = str_random(5) . '-' . $CustomerID . '-' . str_random(5) . '.pdf';
            ...
            \Mail::send('includes.emails.invoice', [
                "CustomerName" => $CustomerName, 'payUrl' => $payUrl
